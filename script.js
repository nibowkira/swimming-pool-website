...
/* ==========================================================================
   Legendary Visualizer Studio 2.0 Logic
   ========================================================================== */
const initVisualizer = () => {
  const stage = document.querySelector('#visualizer-stage');
  const previewImg = document.querySelector('.preview-base');
  const atmosSlider = document.querySelector('#atmosphere-slider');
  const atmosLabel = document.querySelector('#atmosphere-label');
  const hudAtmos = document.querySelector('#hud-atmosphere');
  const hudMaterial = document.querySelector('#hud-material');
  const lightingOverlay = document.querySelector('#lighting-fx');
  const waterOverlay = document.querySelector('#water-caustics');
  const toggleCaustics = document.querySelector('#toggle-caustics');
  ...
  const whatsappBtn = document.querySelector('#visualizer-whatsapp-btn');

  if (!stage) return;

  // 1. Atmosphere Slider Logic
  atmosSlider.addEventListener('input', (e) => {
    ...
    if (val < 30) {
      ...
      overlayBg = 'rgba(255, 100, 0, 0.1)';
    } else if (val > 70) {
      label = 'Dusk';
      filter = 'brightness(0.6) hue-rotate(-20deg)';
      overlayBg = 'rgba(0, 50, 150, 0.2)';
    }

    atmosLabel.textContent = label;
    hudAtmos.textContent = label + ' (' + val + '%)';
    previewImg.style.filter = filter;
    lightingOverlay.style.background = overlayBg;
  });

  // 2. View Mode Toggle (Render vs Blueprint)
  modeBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (mode === 'blueprint') {
        stage.classList.add('mode-blueprint');
      } else {
        stage.classList.remove('mode-blueprint');
      }
    });
  });
  ...
  visualizerBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      visualizerBtns.forEach(b => b.classList.remove('is-active'));
      btn.classList.add('is-active');
      
      const val = btn.dataset.value;
      if (val === 'night') {
        previewImg.style.filter = 'brightness(0.4) contrast(1.1)';
        lightingOverlay.style.background = 'rgba(0, 10, 50, 0.4)';
        hudAtmos.textContent = 'Midnight Glow';
      } else if (val === 'twilight') {
        previewImg.style.filter = 'brightness(0.8) sepia(0.3)';
        lightingOverlay.style.background = 'rgba(200, 100, 0, 0.2)';
        hudAtmos.textContent = 'Golden Hour';
      } else {
        previewImg.style.filter = 'none';
        lightingOverlay.style.background = 'transparent';
        hudAtmos.textContent = 'Midday Sun';
      }
    });
  });
  ...
  textureBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      textureBtns.forEach(b => b.classList.remove('is-active'));
      btn.classList.add('is-active');
      hudMaterial.textContent = btn.dataset.label;
    });
  });

  // 5. Toggle Caustics
  toggleCaustics.addEventListener('change', (e) => {
    waterOverlay.style.display = e.target.checked ? 'block' : 'none';
  });

  // 6. WhatsApp Lead Gen
  whatsappBtn.addEventListener('click', () => {
    const config = {
      atmosphere: hudAtmos.textContent,
      material: hudMaterial.textContent,
      mode: document.querySelector('.mode-btn.is-active').textContent
    };
    
    const message = `Hello Surax Pool! I've designed a custom pool configuration on your website:\n\n` +
                    `- View Mode: ${config.mode}\n` +
                    `- Atmosphere: ${config.atmosphere}\n` +
                    `- Surface: ${config.material}\n\n` +
                    `I'd like to get a quote for this project!`;
    
    window.open(`https://wa.me/251922729374?text=${encodeURIComponent(message)}`, '_blank');
  });
};
